...
% Interplanetary transfer arc from Earth to Mars
...
h_LEO = 300;%input('Quota LEO [km] = ');
delta_V = input('Delta-V [km/s] = ');
...
fprintf('eccesso di velocità iperbolico rispetto alla terra (v_inf o anche deltV INTEPL): %.4f km/s\n', v_inf);
fprintf('velocità eliocentrica in uscita dalla SOI terrestre: %.4f km/s\n', v_inf+ve);
...
fprintf('True anomaly all\''arrivo su Marte: %.2f°\n', rad2deg(theta_star_plus));
...
fprintf('Velocità eliocentrica S/C all''arrivo su Marte (all''ingresso SOI): %.4f km/s\n', v_sc_arr);
fprintf('Velocità eliocentrica di Marte (vm): %.4f km/s\n', vm);
fprintf('Eccesso di v. iperbolico rispetto a Marte (v_infty_Mars): %.4f km/s\n', v_inf_Mars);
...
fprintf('Angolo tra V. eliocentrica S/C e V. marte in ingresso SOI (delta_i): %.4f deg \n', delta_i_deg);
