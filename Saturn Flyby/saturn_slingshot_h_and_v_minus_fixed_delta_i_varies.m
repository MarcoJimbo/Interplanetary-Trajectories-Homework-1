...
delta_i = linspace(30,90,300);    % 3) Angle between SC velocity @SOI encounter and planet velocity [°]
...
Delta_V=abs(v_final-v_minus);

%% Table of the results
fprintf(['Here below are reported the results in the case we fix the altitude of the spacecraft respect to the surface of the planet h and the heliocentric velocity vector of the spacecraft at the encounter v-\n while varying the angle between the velocity vecotr of the planet and the heliocentric velocity vector of the spacecraft at the encounter v-\n\n']);
i=1:length(delta_i);
delta_i_str=arrayfun(@(x) sprintf('%.2f°',x),delta_i,'UniformOutput',false);
...
T=table(i',delta_i_str',v_inf_str',xi_str',v_final_str',delta_f_str',Delta_V_str','VariableNames',{'#','Angle between planet velocity and SC velocity at the encounter','Initial hyperbolic excess velocity v_inf','deflection angle xi','Final heliocentric speed v_final','Final flight angle delta_f','Delta_V exiting Saturn'});
disp(T)
