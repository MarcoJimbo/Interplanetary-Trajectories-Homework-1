...
gamma_max = 0.8 / (9.81 * 1300);
W = gamma_max * 1300; % 1300 kg di m0 

mu_sun = 132712440018; %[km^3/s^2] parametro gravitazionele sole
mu_s = 37931187; %[km^3/s^2] parametro gravitazionale saturno
mu_g = 126686534; %[km^3/s^2] parametro gravitazionale giove

r_s = 58232; %[km] raggio di saturno
r_encelado = 238000; %[km] raggio orbita encelado attorno a saturno

R_start = r_s + 81948; %[km] %generalmente tra 1.000 km e 100.000 km sopra la superficie del pianeta, noi decidiamo di arrivare all'esterno della fascia F degli anelli di Saturno che hanno un raggio di 140180km dal centro di Saturno
...
theta0 = 0; 
vr0 = 0;
vtheta0 = sqrt(mu_s/r0); 

% vincoli finali
rf = r_encelado + 150; 
vrf = 0;
vthetaf = sqrt(mu_s/rf); %thetaf è unparametro libero
...
Pvr0 = 0.150;
Pvtheta0 = 0.90946;
PC0 = -0.7;  %%questo è il moltiplicatore di lagrange associato all'eq.diff aggiuntiva dello stato
t_fin = 14.6*(10^4); %%tempo finale stimato con homann(a detta del professore)
%Cf = gamma_max * t_fin;
Cf = 0.157;
...
H = Pr(i) * vr(i) + Pvr(i) * ( (vtheta(i)^2 / r(i) ) - (mu_s / r(i)^2)) + Pvtheta(i) * (- vtheta(i) * vr(i) / r(i)) + gamma * (norm(Pv) + Pc); %hamilton
...
plot(r_encelado * cos(alpha), r_encelado * sin(alpha), 'Color', [0.7 0.2 0], 'LineWidth', 1.2, 'DisplayName', 'Orbita Encelado');
